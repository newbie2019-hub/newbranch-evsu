   ...
   <div class="row">
    ...
    <div class="row pe-5 ps-3 g-0 mt-3">
     <div class="users-list col-12 col-sm-12 col-md-3 col-lg-3 col-xl-3 p-4">
      ...
      <li v-on:click.prevent="setRecipient(stud.id)" class="cursor-pointer text-dark shadow-none mt-1 fs-6" v-for="(stud, i) in admins" :key="i" :value="stud.id">
       <avatar class="me-2" :username="stud.userinfo.first_name + ' ' + stud.userinfo.last_name" :rounded="true" :size="30" :color="'#fff'" :lighten="100"></avatar>{{ stud.userinfo.first_name }} {{ stud.userinfo.last_name }}
      </li>
     </div>
 ...
 export default {
  ...
  methods: {
   async getMessages(page = 1) {
    const res = await this.$store.dispatch('message/getMessages', { page: page, sort: this.sort });
   },
   async sendMessage() {
